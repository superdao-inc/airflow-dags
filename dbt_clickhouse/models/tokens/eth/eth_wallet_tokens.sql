...
FROM (
	...
	FROM {{ source('eth_data', 'token_transfers_by_token') }} AS transfers
	JOIN {{ ref('eth_contracts_by_address') }} contracts
...
WHERE wallet NOT IN {{ source('eth_data', 'blacklisted_wallets_common') }}
